--[[ TODO
- Change download path if we want this script to auto-update somehow.
- Handle versioning of the script
- Make sure the new area in map.currentArea is set when the area is read, not later
  - find_area or set_area might help with that.
- Change room by doing set_room 
- Remove `lines` as this just stores lines to search for room name.
- What's the move queue used for? If not useful, remove capture_move_cmd
- When calling `capture_room_info`, the `exits` argument needs to be a table of exits.
- `start_mapping` no longer takes the area name as parameter
- double check that the knowledge of map room ids can't help anywhere (prob checking connections)
- remove `random_move` mentions. Should never happen.
- remove map.prompt.room and map.prompt.exits.
--]]
-- Based on Jor'Mox's Generic Map Script v2.0.24
...
-- make sure versions.lua has the latest version in it
    ...
    local function config()
        ...
        --[[ TODO update to the current download path
    ...
    -- TODO: Added roomID and area_name args.
    local function create_room(roomID, name, area_name, exits, dir, coords)
        -- makes a new room with received names and exits
    ...
    local function move_map()
        ...
        if move then
            -- start from exits of prev room (the current room id). The room name should be the new room.
            ...
            elseif force_portal then
                ...
                create_room(map.currentRoom, map.currentName, map.currentArea, map.currentExits, nil, {getRoomCoordinates(map.currentRoom)})
                force_portal = false
            ...
            elseif move == map.configs.lang_dirs['look'] and map.currentRoom and not check_room(map.currentRoom, map.currentName, map.currentExits) then
                ...
                elseif not vision_fail then
                    if map.mapping and move then
                        find_link(map.currentName, map.currentExits, move, map.configs.max_search_distance) -- Never necessary
                    else
                        map.find_me(map.currentName,map.currentExits, move)
                    end
                end
        ...
        if (not vision_fail) and name and exits then
            ...
            for k, _ in pairs(exits) do
                if map.configs.use_translation then
                    local dir = map.configs.translate and map.configs.translate[k]
                    if dir then table.insert(map.currentExits,dir) end
                else
                    table.insert(map.currentExits,k)
                end
            end
    ...
    function map.start_mapping()
        ...
        if not area_name then
            show_err("Room detection not yet working; cannot find area name.")
        end
    ...
    -- TODO: Gut this function once move map has been modified.
    function map.find_me(name, exits, dir, manual)
        ...
        local match_IDs = {}
        table.insert(match_IDs, map.currentRoom)
        
        local rooms = match_IDs
        ...
        -- TODO: come back once I clarify when a new room is created or moved into.
        if not find_portal then
            set_room(map.currentRoom)
            map.echo("Room found, ID: " .. map.currentRoom,true)
    ...
    --[[ TODO: Should never happen; to test and remove.
    ...
    function map.resumeSpeedwalk(delay)
        if #speedWalkDir ~= 0 then
            ...
            tempTimer(delay or 0, function() map.speedwalk() end)
        else
            map.echo("Not currently speedwalking.")
        end
    end
    ...
    function map.eventHandler(event, ...)
        ...
        elseif event == "sysManualLocationSetEvent" then
          set_room(arg[1])
        -- TODO: Change those to reflect new package name.
